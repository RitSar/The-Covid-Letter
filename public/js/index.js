...
$(function() {
  $.getJSON("/jsonworld", function(b) {
    ...
    $.each(b.countries_stat, function(b, d) {
      "India" == d.country_name && ($(".x").text(d.cases), $(".y").text(d.total_recovered), $(".z").text(d.deaths));
  ...
  $.getJSON("/jsonindia", function(b) {
    jsonInd = b;
    $.each(b.state_wise, function(b, e) {
      $("<tr class='here' id=" + e.statecode + "><td >" + e.state + "</td><td>" + e.confirmed + "</td><td>" + e.recovered + "</td><td>" + e.deaths + "</td></tr>").appendTo("#indiadata tbody")
    })
  })
});
$(document).ready(function() {
  $(document).on("click", ".here", function(a) {
    ...
    setTimeout(function() {
      ...
      $.each(jsonInd.state_wise, function(a, c) {
        b == c.statecode && ($(".slotText").text(c.state), $.each(c.district, function(a, c) {
          $("<div class = 'col-4'>" + a + "<p class='text-warning'>Cases: " + c.confirmed + "</p></div>").appendTo("#toAdd")
        }))
      });
      $(".slot").slideDown()
    }, 500)
  ...
  var b = [],
    ...
    m,
    n;
  $.getJSON("https://covidapi.info/api/v1/global/count", function(a) {
    var v = 0;
    $.each(a.result, function(a, c) {
      0 < v && (w.push(c.confirmed - l), x.push(c.deaths - m), y.push(c.recovered - n), g.push(a.substring(5)));
  ...
  var z = [],
    ...
    r,
    t;
  $.getJSON("https://covidapi.info/api/v1/country/IND", function(a) {
    var b = 0;
    $.each(a.result, function(a, c) {
      23 == c.recovered && (c.deaths = 6);
      0 < b && (C.push(c.confirmed - q), D.push(c.deaths - r), E.push(c.recovered - t), h.push(a.substring(5)));
